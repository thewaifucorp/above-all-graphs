...
`write_html` builds `graph.html` from `GRAPH_HTML_TEMPLATE` (`assets/graph.html.template`), substituting placeholders for the page payload (`graph_payload_json`), the source text of indexed files (`source_map_json`, so clicking a node shows the whole file with no server fetch), and the repo name.

Community detection happens here and only here. `normalize_communities` takes `crate::analysis::communities`, folds any community below `MIN_COMMUNITY_MEMBERS` into the dominant community of its own file (a stray helper is not a module and should not spend a palette colour), and renumbers by size so id 0 is the largest. The page used to redo all of that with its own label propagation, which meant the ids in this payload and the ids the page coloured by were different numbers for the same thing.

The page payload is not the same shape as `graph.json`. `graph_payload_json` encodes nodes and edges *columnar* — parallel arrays plus travelling dictionaries for node kind, file path, relation kind, and confidence — because the edge list dominates the page and one edge as an object with four named keys costs roughly four times what four integers cost. It also ships the aggregates the graph's Overview state needs: per community a readable label (`community_label`, the deepest directory every member shares, then the dominant file, then the id), member list, internal/external edge counts, entrypoint count, worst contained confidence, and top-degree representatives; plus inter-community bundles pre-aggregated by relation kind with per-confidence counts. The page decodes it back into the object shape the rest of the script uses. `graph_data_json` keeps the readable object form and is what `graph.json` gets — that file is a public export, not a payload.

`source_map_json` embeds file text up to `SOURCE_EMBED_BUDGET_BYTES`, spending the budget most-referenced-file-first, and reports how many files it had to omit. Past the budget the file viewer says the source is not embedded and shows the path and line instead of opening an empty modal. A single file larger than the whole budget is still embedded, since a viewer that never works is worse. The budgets these numbers answer to are in [graph experience](graph-experience.md). The page embeds vendored copies of sigma.js (`SIGMA_JS`, the WebGL renderer) and graphology (`GRAPHOLOGY_JS`, sigma's graph model) via `include_str!` — bundled at compile time, no CDN, per the house rule that the site is 100% offline. D3 was vendored here until no view needed a whole-repository force layout; it existed only for `d3.quadtree`, and dropping it took 280 KB off every generated page.
