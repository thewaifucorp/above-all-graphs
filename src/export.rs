...
<div class="wrap">
  ...
  <div class="raw">
    ...
    <a href="context.yaml">context.yaml</a>
  </div>
...
fn graph_data_json(nodes: &[Node], edges: &[Edge]) -> Value {
    ...
    json!({
        "nodes": nodes.iter().map(|n| json!({
            ...
            "endLine": n.end_line,
            "community": n.id.and_then(|id| community_by_node.get(&id).copied()),
        })).collect::<Vec<_>>(),
...
fn report_markdown(root: &Path, nodes: &[Node], edges: &[Edge]) -> String {
    ...
    if communities.is_empty() {
        let _ = writeln!(out, "_No multi-node communities detected._\n");
    } else {
        for community in communities.iter().take(20) {
            let names = community
                .members
                .iter()
    ...
    let processes = crate::analysis::processes(nodes, edges);
    let _ = writeln!(out, "## Execution processes\n");
    if processes.is_empty() {
        let _ = writeln!(out, "_No multi-step call processes detected._\n");
    } else {
        for process in processes.iter().take(20) {
            let entrypoint = by_id
